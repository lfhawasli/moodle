...
/**
 * Override the logic for a drawer from the boost theme.
 *
...
define(['jquery', 'core/custom_interaction_events', 'core/log'],
    ...
    var Drawer = function() {
        ...
        // START UCLA MOD: CCLE-7779 - Mobile: Navigation disappears but button stays x.
        if ($("#hamburger-wrapper").length != 0) {
            var state = $("#hamburger-wrapper").attr("aria-expanded");
            if (state == "true") {
    ...
    Drawer.prototype.toggleDrawer = function(e) {
        ...
        if (!open) {
            ...
            // START UCLA MOD: CCLE-7779 - Mobile: Navigation disappears but button stays x.
            hamburger.addClass("change-hamburger-icon");
            page.removeClass("left-gutter");
            // END UCLA MOD: CCLE-7779.
        } else {
            // Close.
            body.removeClass('drawer-open-' + side);
