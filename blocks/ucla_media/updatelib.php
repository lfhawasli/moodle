 ...
 * @param $tablename The moodle DB table name against which to validate the field labels of the CSV data.
 * @note Currently only works with the Bruincast update script at ./bruincast_dbsync.php
 * May cause undefined behaviour if used with other datasets.
 *
 */
function cleanup_csv_data(&$dataarray, $tablename) {
    ...
    $curfields = $DB->get_records_sql("DESCRIBE {$CFG->prefix}" . $tablename);
    ...
    $fielddescriptorsobtained = false;
    $fieldsdescline = -1;
    $totallines = count($incomingdata);

    while (!$fielddescriptorsobtained) {

        $fieldsdescline++;

        if ($fieldsdescline == $totallines) {
            die("\nCould not find any lines that match any field in the DB!\n");
        }

        $filefields = array();

        foreach ($incomingdata[$fieldsdescline] as $tabnum => $fieldname) {

            if (!isset($posfields[$fieldname])) {
                $ignoredfields[] = $fieldname;
            } else {
                $finfields[] = $fieldname;
            }

            $filefields[$fieldname] = $fieldname;
        }

        // Assume that this is the line that we want.
        $fielddescriptorsobtained = true;
    ...
    $dataincoming = array();
    $invalidrestrictions = array();

    for ($line = $fieldsdescline + 1; $line < count($incomingdata); $line++) {

        // Make sure this line has data as we have fields.
        if (count($incomingdata[$line]) != count($filefields)) {
        ...
        foreach ($incomingdata[$fieldsdescline] as $tabnum => $fieldname) {
            ...
            if ('restricted' == $field) {
                ...
                if (!in_array($data, array('Open', 'Restricted', 'See Instructor', 'Online'))) {
                    $invalidrestrictions[] = "Line $line has unknown restriction of $data. Marking it as 'Undefined'."
                            . " Please add it.\n";
                    // Restriction is not in lang file (for now).
                    // Restriction as undefined.
                    $data = 'Undefined';
                }
            }

            $dataincoming[$line][$field] = $data;
        }
    }

    if (!empty($invalidrestrictions)) {
        $invalidrestrictions = implode("\n", $invalidrestrictions);

        $bcastsourceurl = get_config('block_ucla_bruincast', 'source_url');
        $bcastquietmode = get_config('block_ucla_bruincast', 'quiet_mode');

        if (empty($bcastsourceurl) || $bcastquietmode) {
            echo $invalidrestrictions;
        } else {
            ucla_send_mail($bcastsourceurl,
                    'BruinCast data issues (' . date('r') . ')', $invalidrestrictions);
        }
    }
 ...
 * @param string $subjectarea      Default to null.
 * @param string $catnum           Default to null.
 * @param string $secnum           Default to null.
 *
 * @return int      Returns courseid if found, else returns null
 */
function match_course($term, $srs, $subjectarea=null, $catnum=null, $secnum=null) {
    global $DB;
    $retval = null;   // Trying to find courseid that matches params.
    $foundbadsrs = false; // Used to track bad data source.
    ...
    if (!$foundbadsrs) {
        $retval = ucla_map_termsrs_to_courseid($term, $srs);
        if (empty($retval)) {
            ...
            if (!empty($result)) {
                $primarysrs = $result[0]['srs_crs_no'];
            }

            if (!empty($primarysrs) && $srs != $primarysrs) {
                $a->primary_srs = $primarysrs;
                echo get_string('noticediscussionsrs', 'tool_ucladatasourcesync', $a) . "\n";
                $retval = ucla_map_termsrs_to_courseid($term, $primarysrs);
            }
            ...
            if (empty($retval)) {
                ...
                if (empty($results)) {
                    // Bad srs number, so try to match using $subjectarea,
                    // $catnum, (and $secnum).
    ...
    // number (if any).
    if (empty($retval) && !empty($subjectarea) && !empty($catnum)) {
        $sql = 'SELECT  courseid
        ...
        $params['term'] = $term;
        $params['subj_area'] = $subjectarea;
        $params['coursenum'] = $catnum;

        if (!empty($secnum)) {
            $sql .= ' AND rci.sectnum = :sectnum';
            $params['sectnum'] = $secnum;
        }
        ...
        if (count($records) == 1) {
            ...
            $retval = $record->courseid;
        }

        if (!empty($retval) && $foundbadsrs) {
...
function get_reserve_data($table) {
    ...
    foreach ($columns as $columnname => $columninfo) {
        if ($columnname == 'id' || $columnname == 'courseid') {
            continue;
        }
...
function log_ucla_data($func, $action, $notice, $error = '') {
    ...
    $logmessage = empty($error) ? $notice : $notice . PHP_EOL . $error;
    ...
    if (!empty($error)) {
        $contactemail = get_config('contact_email', 'tool_ucladatasourcesync');
        if (!empty($contactemail)) {
            $body = sprintf("Action: %s\nNotice: %s\nError: %s", $action,
                    $notice, $error);
            ucla_send_mail($contactemail, 'Error from ' . $func, $body);
        }
    }
}
