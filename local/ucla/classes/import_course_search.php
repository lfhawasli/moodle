...
class local_ucla_import_course_search extends import_course_search {
    ...
    protected function get_searchsql() {
        ...
        $join   = "  LEFT JOIN {role} r ON (r.shortname = 'editinginstructor' OR r.shortname='supervising_instructor')
                     ...
                     LEFT JOIN {enrol} e ON (e.courseid = c.id AND e.status = :statusenabled)
                     LEFT JOIN {user_enrolments} ue ON (ue.enrolid = e.id AND ue.status = :useractive)
                     LEFT JOIN {user} usr ON (usr.id = ra.userid AND usr.id = ue.userid)";
