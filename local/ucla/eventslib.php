 ...
 * @param type $course
 */
function set_engineering_default($course) {
    $courseid = $course->id;
    ...
    foreach ($reginfos as $reginfo) {
        if ($reginfo->division == 'EN') {
            ...
            course_get_format($courseid)->update_course_format_options(array('hide_autogenerated_content' => true));

            // Add the "Recent activity" and "Upcoming events" blocks upon build and place them above the "Administration" block.
            if ($course->id == SITEID) {
            ...
            $page->blocks->add_block('recent_activity', BLOCK_POS_RIGHT, -10, 0, $pagetypepattern);
            $page->blocks->add_block('calendar_upcoming', BLOCK_POS_RIGHT, -9, 0, $pagetypepattern);
            break;
        }
    }
}