<?php
    ...
    echo $OUTPUT->header();
    require_capability('mod/videoannotation:grade', $module_context);
    ...
    $navlinks[] = array('name' => format_string($videoannotation->name), 'link' => 'view.php?id='.$cm->id, 'type' => 'activityinstance');
    $navlinks[] = array('name' => get_string('submissions', 'videoannotation'), 'link' => '', 'type' => 'activityinstance');
    
    //$navigation = build_navigation($navlinks); //deprecated in 2.7
    $navigation = $PAGE->navbar;

   // print_header_simple(format_string($videoannotation->name), '', $navigation, '', '', true, update_module_button($cm->id, $course->id, $strvideoannotation), navmenu($course, $cm));
...
white-space:nowrap;
    ...
    <div style='text-align: right'><a href="report.php?id=<?php echo $id; ?>"><?php echo get_string('viewreport', 'videoannotation'); ?></a></div>
    ...
    $users_allowed_to_submit = get_users_by_capability($module_context, 'mod/videoannotation:submit', 'u.id', '', '', '', $currentgroup, '', false);
    if ($currentgroup) {
        $group_members = groups_get_members($currentgroup);
        $users = array_intersect(array_keys($group_members), array_keys($users_allowed_to_submit));
    } else {
        $users = array_keys($users_allowed_to_submit);
    }
    ...
    $extrauserfields = get_extra_user_fields_sql($module_context);
   ...
   //     $sql .= ' AND ' . $where;
    ...
    if ($ausers) {
        $grading_info = grade_get_grades($course->id, 'mod', 'videoannotation', $videoannotation->id, array_keys($ausers));
        
        foreach ($ausers as $auser) {
            //$picture = user_picture::fields('u', $extrauserfields);
            //$picture = user_picture::unalias($auser); //to fix deprecations but needs to be tested
            ...
            if (isset($grading_info->items[0]->grades[$auser->id]->str_long_grade))
                $grade = $grading_info->items[0]->grades[$auser->id]->str_long_grade;
            else
                $grade = get_string('notgraded', 'videoannotation');
                
            if (isset($grading_info->items[0]->grades[$auser->id]->str_feedback))
                $comment = $grading_info->items[0]->grades[$auser->id]->str_feedback;
            else
            ...
            if (isset($grading_info->items[0]->grades[$auser->id]->dategraded))
                $timegraded = userdate($grading_info->items[0]->grades[$auser->id]->dategraded);
            else
                $timegraded = get_string('notgraded', 'videoannotation');
            
            $user_or_group_id_str = ($currentgroup ? '&group=' . $currentgroup : '&user=' . $auser->id);
            if (isset($grading_info->items[0]->grades[$auser->id]->dategraded))
                $status = '<a href="' . $CFG->wwwroot . '/mod/videoannotation/view.php?id=' . $id . $user_or_group_id_str . '">' . get_string('update', 'videoannotation') . '</a>';
            else 
                $status = '<a href="' . $CFG->wwwroot . '/mod/videoannotation/view.php?id=' . $id . $user_or_group_id_str . '">' . get_string('grade', 'videoannotation') . '</a>';
