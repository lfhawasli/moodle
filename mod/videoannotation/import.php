<?php
     ...
     *
     *@param mform Form with CSV file
     *@param int $userid The user id
     *@return array An array of the parsed CSV values
     */
    function parse_csv($mform, $userid) {
        ...
        //Check to see that the csv file imported was formatted correctly. Any undefined offsets will mean the file was incorrect syntax
        for($i = 0; $i<sizeof($strings); $i++) {
            for($j = 0; $j<sizeof($strings[$i]); $j++) {
...
class import_form extends moodleform
{
    function definition()
    {
        ...
        $videoannotation = $DB->get_record("videoannotation", array("id"=>$cm->instance));
        ...
        //Do not show the box which allows imports for separate groups when there is a specified clip, because we will import all tags/events/clip for that option
        if ($this->_customdata['groupmode'] == VIDEOANNOTATION_GROUPMODE_GROUP_GROUP || $this->_customdata['groupmode'] == VIDEOANNOTATION_GROUPMODE_GROUP_USER)
            $mform->addElement('select', 'groupselect', get_string('groupselect', 'videoannotation'), $this->_customdata['groupnames']);
...
if ($mform->is_cancelled()) {
    ...
    foreach($strings as $key=>$value)
    {
        ...
        if(isset($importgroups[$groupname]))         //the group data has already been created
        {
            ...
            if(isset($data->tagevents[$tagname]))           //add the event to the group's events
            { 
                if ($event->start != NULL)
    ...
    $runflag = 0; //only insert for one group
    ...
    foreach ($importgroups as $key=>$data)
    {
        ...
        /***  videoannotaion_clips ***/
        if($videoannotation->clipselect == 1) {
            $clip = $DB->get_record('videoannotation_clips', array('videoannotationid'=>$data->videoannotationid, 'userid'=>null, 'groupid'=>null));
        } else if( $videoannotation->groupmode == NOGROUPS) {
            $clip = $DB->get_record('videoannotation_clips', array('videoannotationid'=>$data->videoannotationid, 'userid'=>$userid));
        } else {
            $clip = $DB->get_record('videoannotation_clips', array('videoannotationid'=>$data->videoannotationid, 'groupid'=>$data->groupid));
        }
            if($clip) {
                $clipid = $clip->id;
            } else {
        ...
        foreach($data->tagevents as $key=>$tag)
        {
            $count = 0;
            $tagname = $key;
            //If the video annotation has a specified clip, the tags/events must go across all groups when imported. Otherwise, only import tags/events for one group
		...
		{
                    $olditerator = 0;
		while($olditerator != sizeof($oldtags))             //check if the tagname already exists, case insensitive
		{
		    if(strtolower($tagname) == strtolower($oldtags[$olditerator]->name) && $data->groupid == $oldtags[$olditerator]->groupid)
		    {
			//rename the old tag
			$i = 1;
			$newname = $tagname . $i;
			while($record = $DB->get_record('videoannotation_tags',array('clipid'=>$clipid, 'groupid'=>$data->groupid, 'name'=>addslashes($newname))))
			{
			    $i++;
			    $newname = $tagname.$i;
			}
			...
			//DONE: do some conflict resolution
		    }
                    $olditerator++;
		}
		}
		$tagobj = new stdClass();                   //insert the new tag
