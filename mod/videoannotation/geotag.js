...
/* codeLocation(type, latId, lngId, addressId) ************************
...
function codeLocation(type, latId, lngId, addressId, scopeId, eid) {
    if (typeof google == "undefined")
    {
        //geocoder didnt load correctly, so do nothing
        return -1;
    ...
    switch (type)
    {
        case 'address':
            ...
            geocoder.geocode( {'address': address}, function (results, status) {
                ...
                processGeocode(results, status, latId, lngId, addressId, scopeId, eid);
            });
        break;
        case 'latlng':
...
/* processGeocode(results, status, latId, lngId, addressId) ***************
...
function processGeocode(results, status, latId, lngId, addressId, scopeId, eid) {
    if (status == google.maps.GeocoderStatus.OK)
    {
        ...
        //Everytime the address is reverse geocoded (which always happens upon opening an event) geocode to parse the address into the 'results' array. Always fill out the scope here with the results array and change the selected option.  
