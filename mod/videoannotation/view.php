<?php  // $Id: view.php,v 1.6.2.3 2009/04/17 22:06:25 skodak Exp $
    ...
    /**
     * This page prints a particular instance of videoannotation
     *
    ...
    $isAdmin = is_siteadmin($userid);//has_capability('moodle/site:doanything', $moduleContext);
    ...
    //   * "manage": if groupid given, then use that group, else use the first group in the course; there must be at least one group in the course
    //   * "view": if groupid given, then use that group, else use the first group the user is in; the user must be in this group and there must be at least one group in the course
    ...
    switch ($videoannotation->groupmode) {
        ...
        case VIDEOANNOTATION_GROUPMODE_GROUP_USER:
            ...
            if ($isAdmin or $canManage) {
                ...
                if (!$groups) {
                    error('Must have at least one group');
                }
                if ($groupid) {
    ...
    // See if there is a submission record (created either if the student has submitted something or if the instructor has graded on the activity)
    
    if ($videoannotation->groupmode == NOGROUPS) {
        $submission = $DB->get_record('videoannotation_submissions', array('videoannotationid'=>$videoannotation->id, 'userid'=>$userid, 'groupid'=>null));
    } else {
        $submission = $DB->get_record('videoannotation_submissions', array('videoannotationid'=>$videoannotation->id, 'groupid'=>$groupid));
    }
    ...
    switch ($videoannotation->clipselect) {
        case 0:
            if ($videoannotation->groupmode == NOGROUPS) {
                $clip = $DB->get_record('videoannotation_clips', array('videoannotationid'=>$videoannotation->id, 'userid'=>$userid, 'groupid'=>null));
            } else {
        ...
        case 1:
            $clip = $DB->get_record('videoannotation_clips', array('videoannotationid'=> $videoannotation->id, 'userid'=> null, 'groupid'=>null));
            if (!$clip)
                error("Clip not found for video annotation #{$videoannotation->id} and user #{$userid}");
            break;
            
        default:
            error("Invalid clipselect value {$videoannotation->clipselect} for video annotation #{$videoannotation->id} and user #{$userid}");
    }
    ...
    foreach ($CFG->tna_permalink_url as $idx => $tna_permalink_url) {
        if ($clip and stripos($clip->url, $tna_permalink_url) === 0) {
            @list($uuid, $offset) = explode(',', substr($clip->url, strlen($tna_permalink_url)));
            if ($uuid) {
                $content = file_get_contents($CFG->tna_webservice_url[$idx] . '?action=uuidToFileName&uuid=' . urlencode($uuid));
                $contentobj = json_decode($content);
                if (preg_match('/^(\d{4})\-(\d{2})\-(\d{2})_(\d{2})(\d{2})/', $contentobj->filename, $matches)) {
                    $rtmp_url = $CFG->tna_streamer_url . "/{$matches[1]}/{$matches[1]}-{$matches[2]}/{$matches[1]}-{$matches[2]}-{$matches[3]}/" . basename($contentobj->filename, '.txt') . '.mp4';
                    $clip->url = $rtmp_url;
                    break;
                }
            }
    ...
    if (!optional_param('printable', false, PARAM_BOOL)) {
        ...
        if (has_capability('mod/videoannotation:grade', $moduleContext)) {
            ...
            . $ungraded_submission_count . ' ' . get_string('ungraded', 'videoannotation') . ')</a><br />';
        }
        ...
        ?>
            ...
            <a href='report.php?id=<?php echo $id; ?>&group=<?php echo $groupid; ?>'><?php echo get_string('viewreport', 'videoannotation'); ?></a> |
        ...
        ?>
        <a href='import.php?id=<?php echo $id; ?>&group=<?php echo $groupid; ?>'><?php echo get_string('importannotation', 'videoannotation'); ?></a>
        ...
        if (has_capability('mod/videoannotation:grade', $moduleContext)) {
            ...
            if (optional_param('grade', null, PARAM_INT) != null || optional_param('feedback', 0, PARAM_RAW) != null) {
                ...
                foreach ($gradeuserids as $gradeuserid) {
                    grade_update('mod/videoannotation', $cm->course, 'mod', 'videoannotation', $videoannotation->id, (int) $groupid, array(
                    'userid' => $gradeuserid,
            ...
            <table id="grading" style="display:none;">
                <tr>
                    ...
                    <td><?php echo isset($submission->grade) ? $submission->grade : '(not graded)'; ?></td>
                </tr>
                <tr>
                    <td>Existing feedback: 
                    <td><?php echo isset($submission->gradecomment) ? $submission->gradecomment : '(none)'; ?></td>
                </tr>
                <tr>
                    <td>Score:</td>
                    <td><input type="text" name="grade" value="<?php echo isset($submission->grade) ? $submission->grade : ''; ?>" /></td>
                </tr>
                <tr>
                    <td>Feedback:</td>
                    <td><?php print_textarea(false, 5, 50, 0, 0, 'feedback', strip_tags(isset($submission->gradecomment) ? $submission->gradecomment : ''), $COURSE->id); ?></td>
                </tr>
                <tr>
                    <td></td>
                    <td><input type="hidden" name="user" value="<?php echo $userid; ?>" /><input type="hidden" name="group" value="<?php echo $groupid; ?>" /><input type="submit" value="Grade" /></td>
                </tr>
            </table>
            </form>
            <?php
    ...
    ?>
        ...
        <?php if (!$submission): ?>
        <div><a href="clips.php?id=<?php echo $id; echo ($groupid ? '&group=' . $groupid : ''); ?>"><?php echo get_string('addclip', 'videoannotation'); ?></a></div>
        
        <div><a href="<?php echo $CFG->wwwroot . "/course/view.php?id=" . $course->id; ?>"><?php echo get_string('canceladdclip', 'videoannotation'); ?></a></div>
        <?php endif; ?>
    ...
    ?>
        ...
        <div style='text-align: right'><input type="button" class="playerbutton dark" onclick="window.location='clips.php?id=<?php echo $id; echo ($groupid ? '&group=' . $groupid : ''); ?>'" value="<?php echo get_string('editclip', 'videoannotation'); ?>"/></div>
    ...
    if ($clip): ?>
        ...
        <script src="jquery.text-overflow.min.js" type="text/javascript"></script>
                <script type="text/javascript" src="jwplayer-6.6/jwplayer.js"></script>
        ...
        jQuery(document).ready(function() {
            ...
            var enableControls = function() {
                ...
                //jQuery("#flashPlayerScrubBar").slider("option", "value", Math.round(jwplayer().getPosition() * 1000));
            ...
            //jQuery("#TimelineFrame_Timeline" + timeline.id).find("test").addClass("playerbutton");

            // Create JWPlayer's initialization parameters
            
            var jwplayerParams = {
                players: [
                    { type: "flash", src: "jwplayer-5.3/player.swf" }
                ],
                height: <?php echo $clip->videoheight; ?>,
                width: <?php echo $clip->videowidth; ?>,
                start: <?php echo $clip->playabletimestart; ?>,
                duration: <?php echo $clip->playabletimeend; ?>,
                controlbar: "none",
                autostart: true,
                ...
                events: {
                    ...
                    onTime: function(evt) {
                        ...
                        for (var idx in timeline.events) {
                            if (timeline.events[idx].getStartTime() <= timeline.getCurrentTime() && timeline.getCurrentTime() < timeline.events[idx].getEndTime())
                        ...
                        for (var idx in currentPlayingEvents) {
                            if (timeline.inArray(currentPlayingEvents[idx], prevPlayingEvents, function (a,b) {return a.getId() == b.getId();}) === undefined) {
                        ...
                        for (var idx in prevPlayingEvents) {
                            if (timeline.inArray(prevPlayingEvents[idx], currentPlayingEvents, function (a,b) {return a.getId() == b.getId();}) === undefined) {
        ...
        <table align="center" style="padding: 10px; width: 86%;">
                        ...
                        <div id="flashPlayerMute" class="IconUnmute" title="Click to mute/unmute" style="float: left; width: 16px; height: 16px; background-image: url(jquery-ui-1.8.2.custom/css/ui-lightness/images/ui-icons_228ef1_256x240.png);"></div>
    ...
    and !has_capability('mod/videoannotation:grade', $moduleContext)) {
        ...
        if ($submission) {
            ...
            <div>Feedback:</div>
            <div>Score: <?php if (isset($submission->grade)) echo $submission->grade; ?></div>
            <div>Comment: <?php if (isset($submission->gradecomment)) echo $submission->gradecomment; ?></div>
