    ...
    return function (fp) {
        var hooks = __assign({
            onReady: function () {
                ...
                var timezoneName = (new Date(fp.altInput.value)).dst() ? "PDT" : "PST";
                /*
            ...
            onChange: function(dateObj) {
                // Just hardcoded to PST/PDT.
                var dst = (new Date(dateObj)).dst();
                timezoneContainer.innerHTML = (dst)? "Timezone: PDT" : "Timezone: PST";
            }
        }, {});
        return hooks;
    };
...
// Check whether a date is in DST or not. Source: https://stackoverflow.com/a/30280636.
