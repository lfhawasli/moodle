...
Y.extend(SHORTFORMS, Y.Base, {
    ...
    set_state_all: function(e) {
        ...
        this.update_btns();
        // START UCLA MOD: CCLE-5791 - Expand all should include expanding Show more... section.
        var showset = this.form.all('div.fitem.advanced');
        showset.each(function(node) {
            node.addClass('show');
        });

        var fieldset = this.form.all('fieldset.containsadvancedelements');
        fieldset.each(function(node) {
            var statuselement = Y.one('input[name=mform_showmore_' + node.get('id') + ']');
            statuselement.set('value', 1);
            var togglers = node.all('.moreless-toggler');
            togglers.addClass('moreless-less');
            togglers.setHTML(M.util.get_string('showless', 'form'));

        });

        var adv = this.form.all('img.adv');
        adv.set('style.display', 'none');
        // END UCLA MOD: CCLE-5791.
    },
