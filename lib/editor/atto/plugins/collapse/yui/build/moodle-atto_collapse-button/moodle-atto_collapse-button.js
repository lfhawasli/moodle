...
Y.namespace('M.atto_collapse').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _setVisibility: function(button, visibility) {
        ...
        if (visibility) {
            ...
            this._toggleButtonContent(button, 'expanded');
            ...
            // START UCLA MOD: CCLE-9001 - Improve UI of "More" button in Atto.
            this._toggleButtonContent(button, 'collapsed');
            // END UCLA MOD: CCLE-9001.
        }

    },
    // START UCLA MOD: CCLE-9001 - Improve UI of "More" button in Atto.
    _toggleButtonContent: function(button, content) {
        
        var icon = $('.atto_collapse_button').children('.icon');
        var text = $('.atto_collapse_button').children('.buttontext')[0];
        ...
        if (content == 'expanded') {
            ...
            button.prepend('<span class="buttontext">' + M.util.get_string('buttontext_less', PLUGINNAME) + '</span>');
        } else {
            icon.addClass('fa-caret-right');
            icon.removeClass('fa-caret-down');
            button.prepend('<span class="buttontext">' + M.util.get_string('buttontext_more', PLUGINNAME) + '</span>');
        }
    }
    // END UCLA MOD: CCLE-9001.
}, {
